...
sampling uses CUDA once embeddings are ready or cached. The main example script
uses committed cached embeddings from `example_embeds/`, so a fresh clone should
reach GPU sampling immediately.
